#tag Class
Protected Class PrintRect
Inherits PAF_PrintKit.PrintItem
	#tag Method, Flags = &h0
		Sub ApplyStyle(oStyle as PAF_PrintKit.PrintStyle)
		  me.CornerWidth = oStyle.CornerWidth
		  me.CornerHeight = oStyle.CornerHeight
	...
	#tag Method, Flags = &h0
		Sub Clone(item as PAF_PrintKit.PrintItem)
		  ...
		  me.ShortsStyle = PAF_PrintKit.PrintRect(item).ShortsStyle
		  me.CornerWidth = PAF_PrintKit.PrintRect(item).CornerWidth
		  me.CornerHeight = PAF_PrintKit.PrintRect(item).CornerHeight
		  me.LineColor = PAF_PrintKit.PrintRect(item).LineColor
		  me.LineWidth = PAF_PrintKit.PrintRect(item).LineWidth
	...
	#tag Method, Flags = &h0
		Sub Draw(g as Graphics, Scale as Double, xOffset As Double, yOffset As Double)
		  Dim oRect As New RoundRectShape
		  
		  // drawn as an object2d rectshape which has its origin at the center
		  ...
		  oRect.Height = Height
		  oRect.Border = 100
		  oRect.CornerWidth = CornerWidth
		  oRect.CornerHeight = CornerHeight
		  oRect.BorderWidth = LineWidth
		  orect.BorderColor = LineColor
		  oRect.FillColor = FillColor
		  
		  g.DrawObject(oRect, xPos, yPos)
	...
	#tag Method, Flags = &h0
		Function GetTemplate(oDBWrapper as PAF_DatabaseKit.DBWrapper) As PAF_PropertyListKit.ListItem()
		  ...
		  Items.Append New PAF_PropertyListKit.TextProperty("CornerWidth",me.CornerWidth)
		  Items.Append New PAF_PropertyListKit.TextProperty("CornerHeight",me.CornerHeight)
	...
	#tag Method, Flags = &h0
		Function SerializeJSON() As JSONItem
		  ...
		  jOut.Value("cornerwidth") = me.CornerWidth
		  jOut.Value("cornerheight") = me.CornerHeight

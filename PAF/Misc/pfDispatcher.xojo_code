...
Protected Module pfDispatcher
	#tag Method, Flags = &h0
		Function ErrorCodeToString(code as XojoScript.Errors) As String
		  ...
		  Return errors.Lookup( code, "Unknown Script Error" )
		End Function
	#tag EndMethod

	#tag Method, Flags = &h0
		Sub listen(extends onl as pfReceiver, keys() as string)
		  dim n,l as integer
		  n = ubound( keys )
		  for l=0 to n
		    register( keys( l ), onl )
	...
	#tag Method, Flags = &h1
		Protected Sub register(key as string, inObject as pfReceiver)
	...
	#tag Method, Flags = &h0
		Sub stopListening(extends onl as pfReceiver)
		  if notificationSorter <> nil then
		    ...
		    if receivers = nil then
		      ...
		      for l=1 to n
		        ...
		        if w <> nil and w.value <> nil then
		          pfReceiver( w.value ).notificationReceived( key, value, sender )
	...
	#tag Note, Name = Credit
		Code For pfDispatcher and pfReceiver from
		Sam Rowlands Xojo Forums and XDev Magazine article
		Ohanaware.com
